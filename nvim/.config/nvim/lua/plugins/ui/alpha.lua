...
local function alpha_dashboard_layout()
  ...
  lazycache.menu = function()
    return {
      ...
      button('<CR>',    '󰑓  Load CWD Session',            '<cmd>lua require("persistence").load()<CR>'),
      button('.',       '  Load Last Session',           '<cmd>lua require("persistence").load( {last = true} )<CR>'),
      button('/',       '󱦞  Select Session',              '<cmd>lua require("persistence").select()<CR>'),
